...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  cut1->Set( 100443, 0 );
  cut1->Set( "cc_morg", 0 );
  cut1->Set( "dzll3d",      0         ); // for calib (cc-MC)
  cut1->Set( "dzll3dcalib", 1, 0.0190 ); // for calib (cc-MC)
  ...
  cut2->Set( "cc_morg", 0 );
  cut2->Set( 100443, 0 );
  cut2->Set( "dzll3d",      0         ); // for calib (cc-MC)
  cut2->Set( "dzll3dcalib", 1, 0.0190 ); // for calib (cc-MC)
  TCut cut_mm = cut2->Output();
  MCut_array* cut3 = new MCut_array( branch_table() );

  //TCut cut_kfbcl = "kfbcl>1.0e-18";
  TCut cut_kfbcl = "kfbclcalib>1.0e-18"; // for calib (cc-MC)
  // --------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------
  const Char_t* outfile = Form( "%s%s_cut3.root",outdir,basename.c_str() );

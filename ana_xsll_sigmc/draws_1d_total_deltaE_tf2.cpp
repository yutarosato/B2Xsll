...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  strcpy( add_cut[0], (char*)sTmp.str().c_str() ); // false except q2=true, fl=true
  sTmp.str("");
  sTmp.clear();
  sTmp << "self!=1 && ";
  sTmp << makeCut_q2fl( 1,1 );
  strcpy( add_cut[1], (char*)sTmp.str().c_str() ); // false with q2=true, fl=true
  sTmp.str("");
  sTmp.clear();
  add_cut[2] = "self==1";
  
  //using namespace Mbc;
  using namespace deltaE_wide;
  ...
  if( fl_mode_ll==1 ){
    Deco( hist[0], 3,  9,  9 ); // yellow
    Deco( hist[1], 3, 15, 15 ); // light blue
    Deco( hist[2], 3,  3,  3 ); // blue
  }else if( fl_mode_ll==0 ){
    Deco( hist[0], 3,  9,  9 ); // yelloe
    Deco( hist[1], 3, 13, 13 ); // light red
    Deco( hist[2], 3,  2,  2 ); // red
  }
  ...
  legend1->AddEntry( hist[2],"True",  "P" );
  legend1->AddEntry( hist[1],"False with q^{2}=t,fl=t)",    "P" );
  legend1->AddEntry( hist[0],"False except q^{2}=t,fl=t)",  "P" );
  legend1->Draw();
  // +++++++ tlegend2 ++++++++++++++++++++++++++++++++++
    c1->cd(2);
  ...
  if( flag_save ){
    c1->Print( Form("pic/total_%s_tf2_lep%d_set%s.eps", axis, fl_mode_ll, setname) );
    c1->Print( Form("pic/total_%s_tf2_lep%d_set%s.root",axis, fl_mode_ll, setname) );
  }

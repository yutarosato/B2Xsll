...
set indir  = "hbk_single_nb/hbk_orgksfw_vtxcl_fmiss1_lrnb/"
set outdir = "hbk_single_nb/hbk_orgksfw_vtxcl_fmiss1_weight/"
...
foreach exp ($list_exp)
   foreach stream ($list_stream)
      foreach type ($list_type)
         bsub -q s ./exe_mk_weight_single.sh $indir $outdir $exp $stream $type
      end
   end
end

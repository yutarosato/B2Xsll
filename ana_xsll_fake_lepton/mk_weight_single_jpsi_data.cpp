...
void Get_fake_pi( Double_t c, Double_t p, Int_t fl_mode_ll, Double_t& fake, Double_t& fakeE ){
  ...
  if( fl_mode_ll==1 && xbin==3 && ybin==1 ){
    fake  = (hist_fl[fl_mode_ll]->GetBinContent(2,1) + hist_fl[fl_mode_ll]->GetBinContent(4,1) )/2.0;
    fakeE = sqrt( hist_flE[fl_mode_ll]->GetBinContent(2,1)*hist_flE[fl_mode_ll]->GetBinContent(2,1)
		  +hist_flE[fl_mode_ll]->GetBinContent(4,1)*hist_flE[fl_mode_ll]->GetBinContent(4,1) )/2.0;
  }
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  TCut cut_lrnb = "1";
  // --------------------------------------------------------------------
  const Char_t* outfile = Form( "%s/RD_e0%s_caseB_single_jpsi_weight.root",outdir, exp );

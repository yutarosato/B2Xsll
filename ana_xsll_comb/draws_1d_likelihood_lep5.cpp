...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  const Char_t*  indir_sig[2]       = { "", Form("NB_lep5/hbk/hbk_%s_bcs_merge/",tag) };
  const Char_t*  indir_bkg[2]       = { "", Form("NB_lep5/hbk/hbk_%s_bcs/",      tag) };
  ...
  c1->Update();
  if( flag_save ){
    c1->Print(     Form("pic/1d_%s_lep5_%s_s0%s_set%s.eps",  lrnb[fl_lrnb],tag,stream,setname)             );
    TFile outfile( Form("pic/1d_%s_lep5_%s_s0%s_set%s.root", lrnb[fl_lrnb],tag,stream,setname), "RECREATE" );
    c1->Write();

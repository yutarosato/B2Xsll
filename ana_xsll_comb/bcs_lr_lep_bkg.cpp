...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  std::cout << chain_B->GetEntries() << std::endl;
  ...
  if( m.size()==0 ){
    ...
    if( fl_rd ) sTmp << Form( "%s/RD_e0%s_caseB_bcs.root",             outdir,       expno         ); // [RD]
    else        sTmp << Form( "%s/gMC_%s_e0%s_s0%d_caseB_bcs.root",    outdir, type, expno, stream ); // [gMC]
    Char_t outfile[1024];

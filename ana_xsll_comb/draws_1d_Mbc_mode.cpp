...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(argc==8 || argc==9) ) std::cerr << "wrong input" << std::endl
    					<< " Usage : ./draws_1d_Mbc (char*)stream (char*) setname (double)used_nstream (double)used_nset (int)fl_mode_ll (int)fl_q2 (int)fl_mode_xs [(int)fl_appRun]" << std::endl
					<< std::endl, abort();
  ...
  for( Int_t i=0; i<nbkgtype; i++ ){
    sTmp << makeCut_mode_category( fl_mode_xs ) << " && ";
    sTmp << LRNB_cut;
  ...
  waku->SetTitle( Form("%s (lep=%d,q2=%d,xs=%d,stream=%s,set=%s)", axis,fl_mode_ll,fl_q2,fl_mode_xs,stream,setname) );
  waku->Draw();
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_comb_m%dm_lep%d_q2_%d_s0%s_set%s.eps",  axis, fl_mode_xs, fl_mode_ll, fl_q2, stream, setname) );
    c1->Print( Form("pic/%s_comb_m%dm_lep%d_q2_%d_s0%s_set%s.root", axis, fl_mode_xs, fl_mode_ll, fl_q2, stream, setname) );
  }

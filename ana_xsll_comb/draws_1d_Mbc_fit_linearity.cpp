...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  Int_t          evt_cnt[6]      = {0};
  const Double_t evt_max[6]      = {3.1, 8.8, 148.1, 2.6, 6.7, 162.6}; // [false except(t,t), false with(t,t), true]x[ee,mm] // afb-modes
  //const Double_t evt_max[6]      = {4.4, 9.1, 178.6, 3.6, 6.9, 185.7}; // [false except(t,t), false with(t,t), true]x[ee,mm] // all-modes
  ...
  for( Int_t j=0; j<Nchain; j++ ){
	...
	if     ( j== 3 && evt_cnt[0] >= used_nstream*((Double_t)incl_ratio/100.0)*evt_max[0] ){ std::cout << "BREAK[0]" << std::endl; break; }
	else if( j== 4 && evt_cnt[1] >= used_nstream*((Double_t)incl_ratio/100.0)*evt_max[1] ){ std::cout << "BREAK[1]" << std::endl; break; }
	else if( j== 5 && evt_cnt[2] >= used_nstream*((Double_t)incl_ratio/100.0)*evt_max[2] ){ std::cout << "BREAK[2]" << std::endl; break; }
	else if( j== 9 && evt_cnt[3] >= used_nstream*((Double_t)incl_ratio/100.0)*evt_max[3] ){ std::cout << "BREAK[3]" << std::endl; break; }
	else if( j==10 && evt_cnt[4] >= used_nstream*((Double_t)incl_ratio/100.0)*evt_max[4] ){ std::cout << "BREAK[4]" << std::endl; break; }
	else if( j==11 && evt_cnt[5] >= used_nstream*((Double_t)incl_ratio/100.0)*evt_max[5] ){ std::cout << "BREAK[5]" << std::endl; break; }
	for( Int_t i=0; i<Nroohist; i++ ){
	  ...
	  if( add[rooad[i]][j] ){
	    ...
	    if     ( flag_scale && fl_sb[j]==1 ){
	      if( tmp_rnd > 0.92 ){
		data[i]->add( RooArgSet(*obs[i]), 1.0 );
		if     ( j== 3 && i==Nroohist-1) evt_cnt[0]++;
		else if( j== 4 && i==Nroohist-1) evt_cnt[1]++;
		else if( j== 5 && i==Nroohist-1) evt_cnt[2]++;
		else if( j== 9 && i==Nroohist-1) evt_cnt[3]++;
		else if( j==10 && i==Nroohist-1) evt_cnt[4]++;
		else if( j==11 && i==Nroohist-1) evt_cnt[5]++;
	      }
	    }else if( flag_scale && fl_sb[j]==0 ) data[i]->add( RooArgSet(*obs[i]), 1.0 );
  ...
  for( Int_t i=0; i<6;        i++ ) std::cout << Form("< evt_cnt %d> ", i )    << evt_cnt[i] << std::endl;
  ...
  for( Int_t i=0; i<Nroohist; i++ ){
    ...
    if( sel_fun== 15 ){
      ...
      arg_shape[i] = new RooRealVar( "shape", "argus shape parameter", tmp_shape, tmp_shape-5.0, tmp_shape+5.0 );
  ...
  for( Int_t i=0; i<Nroohist; i++ ){
    ...
    Double_t init_sn_ratio = 0.10*incl_ratio/100.0;
    
    //if( i==0 ) nsig[i] = new RooRealVar ( "N_{sig}", "N_{sig}",   init_sn_ratio  *data[i]->sumEntries(),  (init_sn_ratio-0.10)*data[i]->sumEntries(), (init_sn_ratio+0.10)*data[i]->sumEntries() );
    //if( i==0 ) nbkg[i] = new RooRealVar ( "N_{bkg}", "N_{bkg}", (1-init_sn_ratio)*data[i]->sumEntries(),  (0.90-init_sn_ratio)*data[i]->sumEntries(), (1.10-init_sn_ratio)*data[i]->sumEntries() );
    Double_t tmp_nsig;
    if     ( i==0 ) tmp_nsig = (156.9*incl_ratio/100+33.7)*used_nstream;
    else if( i==1 ) tmp_nsig = (169.3*incl_ratio/100+34.3)*used_nstream;
    else if( i==2 ) tmp_nsig = (326.2*incl_ratio/100+68.0)*used_nstream;
    nsig[i] = new RooRealVar ( "N_{sig}", "N_{sig}", tmp_nsig  ,                     tmp_nsig-100,                       tmp_nsig+100 );
    nbkg[i] = new RooRealVar ( "N_{bkg}", "N_{bkg}", data[i]->sumEntries()-tmp_nsig, data[i]->sumEntries()-tmp_nsig-100, data[i]->sumEntries()-tmp_nsig+100 );
    pdf[i]  = new RooAddPdf( Form("pdf%d",i), Form("pdf%d",i), RooArgList(*gauss[i],*modargus[i]), RooArgList(*nsig[i], *nbkg[i]) );
    if( sel_fun==15 || sel_fun==151 ) fit_result[i] = pdf[i]->fitTo( *data[i], Extended(), SumW2Error(kFALSE), Save(true) );
  }
  ...
  for( Int_t i=0; i<Nroohist; i++ ){
    ...
    std::cout << std::setw( 5) << std::right << incl_ratio/100.0                                           << " "
	      << std::setw(10) << std::right << nsig[i]->getVal()                            /used_nstream << " "
	      << std::setw(10) << std::right << nsig[i]->getPropagatedError( *fit_result[i] )/used_nstream << " "
	      << "  HOGE" << i 
	      << "  FUNC" << sel_fun

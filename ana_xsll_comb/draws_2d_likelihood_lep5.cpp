...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  const Char_t*  indir_sig[2]       = { "", Form("NB_lep5/hbk/hbk_%s_%s_bcs_merge/",tag,bcs) };
  const Char_t*  indir_bkg[2]       = { "", Form("NB_lep5/hbk/hbk_%s_%s_bcs/",      tag,bcs) };
  //const Char_t*  indir_sig[2]       = { "", Form("NB_lep5/hbk_5body/hbk_%s_%s_bcs_merge/",tag,bcs) }; // remove 5-body modes
  //const Char_t*  indir_bkg[2]       = { "", Form("NB_lep5/hbk_5body/hbk_%s_%s_bcs/",      tag,bcs) }; // remove 5-body modes
  ...
  for( int i=0; i<Nchain; i++ ) {
    ...
    //sTmp << " && ( rm_xs!=401 && rm_xs!=410 && rm_xs!=1301 && rm_xs!=1310 )"; // remove 5-body modes
  ...
  c3->Update();
  if( flag_save ){
    c1->Print(     Form("pic/2d_%s_lep5_%s_%s_s0%s_set%s_c1.eps", lrnb[fl_lrnb],tag,bcs,stream,setname)             );
    c2->Print(     Form("pic/2d_%s_lep5_%s_%s_s0%s_set%s_c2.eps", lrnb[fl_lrnb],tag,bcs,stream,setname)             );
    c3->Print(     Form("pic/2d_%s_lep5_%s_%s_s0%s_set%s_c3.eps", lrnb[fl_lrnb],tag,bcs,stream,setname)             );
    TFile outfile( Form("pic/2d_%s_lep5_%s_%s_s0%s_set%s.root",   lrnb[fl_lrnb],tag,bcs,stream,setname), "RECREATE" );
    c1->Write();
    c2->Write(); 
    c3->Write();

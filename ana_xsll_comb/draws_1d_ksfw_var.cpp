...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(argc==6 || argc==7) ) std::cerr << "wrong input" << std::endl
    					<< " Usage : ./draws_1d_ksfw_var (char*)stream (char*) setname (int)fl_mode_ll (int)fl_xs_region (int)fl_ksfw_region [(int)fl_appRun]" << std::endl
  ...
  std::stringstream common_cut;
  if     ( fl_xs>0 ) common_cut << "xs_m>1.1";
  else if( fl_xs<0 ) common_cut << "xs_m<1.1";
  else               common_cut << " 1 ";
  if     ( fl_ksfw == 0 ) common_cut << " && ( k0mm2 <-0.5 )";
  else if( fl_ksfw == 1 ) common_cut << " && ( k0mm2 >=-0.5 && k0mm2 < 0.3 )";
  else if( fl_ksfw == 2 ) common_cut << " && ( k0mm2 >= 0.3 && k0mm2 < 1.0 )";
  else if( fl_ksfw == 3 ) common_cut << " && ( k0mm2 >= 1.0 && k0mm2 < 2.0 )";
  else if( fl_ksfw == 4 ) common_cut << " && ( k0mm2 >= 2.0 && k0mm2 < 3.5 )";
  else if( fl_ksfw == 5 ) common_cut << " && ( k0mm2 >= 3.5 && k0mm2 < 6.0 )";
  else if( fl_ksfw == 6 ) common_cut << " && ( k0mm2 >= 6.0 )";
  ...
  for( Int_t i=0; i<nbkgtype; i++ ){
    strcpy( add_cut[i], (char*)common_cut.str().c_str() ); // false except q2=true, fl=true
  }

  
  sTmp << "self!=1 &&";
  sTmp << makeCut_q2fl( 1,1,1 );
  sTmp << " && " << common_cut.str().c_str();
  ...
  waku->SetTitle( Form("%s (lep=%d,xs=%d,ksfw=%d,stream=%s,set=%s)", axis,fl_mode_ll,fl_xs,fl_ksfw,stream,setname) );
  waku->Draw();
  for(Int_t i=0; i<Nhist; i++ ){
    if( i==2 ) continue;
    if( i==3 ) continue;
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_lep%d_xs_%d_ksfw_%d_s0%s_set%s.eps",  fname, fl_mode_ll, fl_xs, fl_ksfw, stream, setname) );
    c1->Print( Form("pic/%s_lep%d_xs_%d_ksfw_%d_s0%s_set%s.root", fname, fl_mode_ll, fl_xs, fl_ksfw, stream, setname) );
  }

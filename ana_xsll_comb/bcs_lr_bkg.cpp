...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( argc != 7 ){
    std::cerr << "wrong input" << std::endl
	      << "Usage : ./bcs_lr_bkg (char*)expno (int)stream (char*)type (char*)indir (char*)outdir (char*)brname" << std::endl
	      << std::endl;
    abort();
  }
  ...
  tree_B->SetBranchAddress( brname,    &lr      );
  ...
  if( m.size()==0 ){
    std::cout << std::setw(7) << std::right << chain_B->GetEntries() << " " // total
	      ...
	      << 0 << " %(BCS)"
	      << std::endl;
    const Char_t* outfile = Form( "%sgMC_%s_e0%s_s0%d_caseB_bcs.root",outdir, type, expno, stream );

...
const Double_t fraction     = 1.00; // should change file_eff_lep?
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  Int_t    fl_xsid    = 0; // 0(all), 1(K), 2(K*), 3(Xs)
  Char_t*  fl_A7      = argv[2];
  Char_t*  fl_A9      = argv[3];
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    //sTmp << "~/Store/ewp/ana/data/gen/hbk1/hbk/hbk_" << fl_A7 << "_" << fl_A9 << "_" << fl_A10 << "_set" << setname << "/Gen_";
    sTmp << "~/Store/ewp/ana/data/gen_xsspin/hbk1/hbk/hbk_" << fl_A7 << "_" << fl_A9 << "_" << fl_A10 << "_set" << setname << "/Gen_";
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    ...
    for( Int_t xsid=0; xsid<6; xsid++ ){    
      add_cut[i][xsid] = new Char_t[4096];
      ...
      strcpy( add_cut[i][xsid], (char*)sTmp.str().c_str() );
      sTmp.str("");
      sTmp.clear();
    }
  }
 ...
 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  TFile* file_eff_lep0  = new TFile( "eff_xsspin_fr100/2d_q2_theta_lep0_xsid0_setA-U.root" );
  TFile* file_eff_lep1  = new TFile( "eff_xsspin_fr100/2d_q2_theta_lep1_xsid0_setA-U.root" );
  //TFile* file_eff_lep0  = new TFile( "eff_xsspin/2d_q2_theta_lep0_xsid0_setA-U.root" );
  //TFile* file_eff_lep1  = new TFile( "eff_xsspin/2d_q2_theta_lep1_xsid0_setA-U.root" );
  ...
  for( Int_t j=0; j<Nchain; j++ ){
    ...
    if( flag_xsspin ){
      ...
      if( fl_xsid==1 || fl_xsid==2 ) chain[j]->GetTree()->Project( Form("tmphist%d",j), axis[1], add_cut[j][fl_xsid] );
      else if( fl_xsid==0 ){
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("%s && gm_fl_xs<0",          add_cut[j][0])                   ); tmphist[j]->Add( tmp2hist ); // K(*)
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("(%s)*(1.0+ ( Xs_m-(%f))*(%f))", add_cut[j][4], center, fraction) ); tmphist[j]->Add( tmp2hist ); // Xs-spin0
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("(%s)*(1.0+ (-Xs_m+(%f))*(%f))", add_cut[j][5], center, fraction) ); tmphist[j]->Add( tmp2hist ); // Xs-spin1
      }else if( fl_xsid==3 ){
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("(%s)*(1.0+ ( Xs_m-(%f))*(%f))", add_cut[j][4], center, fraction) ); tmphist[j]->Add( tmp2hist );
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("(%s)*(1.0+ (-Xs_m+(%f))*(%f))", add_cut[j][5], center, fraction) ); tmphist[j]->Add( tmp2hist );
      }else if( fl_xsid==4 ){
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("(%s)*(1.0+ ( Xs_m-(%f))*(%f))", add_cut[j][4], center, fraction) ); tmphist[j]->Add( tmp2hist );
      }else if( fl_xsid==5 ){
	chain[j]->GetTree()->Project( "tmp2hist", axis[1], Form("(%s)*(1.0+ (-Xs_m+(%f))*(%f))", add_cut[j][5], center, fraction) ); tmphist[j]->Add( tmp2hist );
      ...
      delete tmp2hist;
    }else chain[j]->GetTree()->Project( Form("tmphist%d",j), axis[1], add_cut[j][fl_xsid] );
  }
  ...
  legend->SetHeader( Form("lep%d, xsid%d, %s, %s, %s", fl_mode_ll, fl_xsid, fl_A7, fl_A9, fl_A10) );
  ...
  c1->Update();
  if( flag_save ) c1->Print( Form("pic/2d_gen_q2_theta_correction_table_lep%d_xsid%d_set%s_%s_%s_%s.eps", fl_mode_ll, fl_xsid, setname, fl_A7, fl_A9, fl_A10) );

...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(argc==4 || argc==5) ) std::cerr << "wrong input" << std::endl
					<< " Usage : ./draws_1d_Mbc_fit (int)rm_xs (int)fl_mc (char*)setname"
					<< std::endl, abort();
  const Int_t   rm_xs              = atoi(argv[1]);
  ...
  for( Int_t i=0; i<Nhist; i++ ){
    if(      i==0 ) hist[i] = new TH1D( Form("hist%d",i), Form("m%dm, ee",        rm_xs), xbin,offset+xmin,offset+xmax );
    else if( i==1 ) hist[i] = new TH1D( Form("hist%d",i), Form("m%dm, #mu#mu",    rm_xs), xbin,offset+xmin,offset+xmax );
    else if( i==2 ) hist[i] = new TH1D( Form("hist%d",i), Form("m%dm, ee+#mu#mu", rm_xs), xbin,offset+xmin,offset+xmax );
  ...
  c1->Update();
  c1->Print( Form("pic/Mbc_fit_mode_m%dm_mc%d_set%s.eps", rm_xs, fl_mc, setname) );

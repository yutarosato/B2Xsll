...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  cut1->Set( "Mbc",     0 );
  cut1->Set( "de",      0 );
  cut1->Set( "xs_m",    0 ); // for switch
  cut1->Set(    441,    0 );
  cut1->Set(    443,    0 );
  cut1->Set( 100443,    0 );
  cut1->Set( "dzll3d",  0 ); // for switch
  ...
  cut2->Set( "Mbc",     0 );
  cut2->Set( "de",      0 );
  cut2->Set( "xs_m",    0 ); // for switch
  cut2->Set(    441,    0 );
  cut2->Set(    443,    0 );
  cut2->Set( 100443,    0 );
  cut2->Set( "dzll3d",  0 ); // for switch
  ...
  sTmp << "("
       << "( rm_l==1 && ( "
       << make_cut_window( "cc_morg", -0.40, PDGmass::jpsi,  0.15 ) << " || "
       << make_cut_window( "cc_morg", -0.25, PDGmass::psi2s, 0.10 ) << " || "
       << make_cut_window( "cc_m",    -0.40, PDGmass::jpsi,  0.15 ) << " || "
       << make_cut_window( "cc_m",    -0.25, PDGmass::psi2s, 0.10 )
       << " ) )"
       << " || "
       << "( rm_l==0 && ( "
       << make_cut_window( "cc_morg", -0.25, PDGmass::jpsi,  0.10 ) << " || "
       << make_cut_window( "cc_morg", -0.15, PDGmass::psi2s, 0.10 ) << " || "
       << make_cut_window( "cc_m",    -0.25, PDGmass::jpsi,  0.10 ) << " || "
       << make_cut_window( "cc_m",    -0.15, PDGmass::psi2s, 0.10 )
       << " ) )"
       << " )";

  TCut cut_cc = sTmp.str().c_str();
  sTmp.str("");
  sTmp.clear();
  ...
  if( strstr(indir, "emu")!=NULL ){
    fl_emu = 1; //emu
    //std::cout << "[  emu   ] ";
    tree_B = chain_B->CopyTree( cut_emu && cut_kfbcl && cut_cc );
  }else{
    fl_emu = 0; // ee or mm
    //std::cout << "[ee or mm] ";
    tree_B = chain_B->CopyTree( (cut_ee || cut_mm) && cut_kfbcl && cut_cc );
      }
  ...
  std::cout << Form( "fl_rd(%d), fl_emu(%d) : ", fl_rd, fl_emu)
	    ...
	    << std::endl;
  // --------------------------------------------------------------------
  if     ( fl_rd && fl_emu ) sTmp << Form( "%s/RD_e0%s_caseB_emu_mergecut4.root",         outdir,       exp         ); // [RD,    emu    ]
  else if( fl_rd           ) sTmp << Form( "%s/RD_e0%s_caseB_mergecut4.root",             outdir,       exp         ); // [RD,  ee or mm ]
  else if(          fl_emu ) sTmp << Form( "%s/gMC_%s_e0%s_s0%d_caseB_emu_mergecut4.root",outdir, type, exp, stream ); // [gMC,   emu    ]
  else                       sTmp << Form( "%s/gMC_%s_e0%s_s0%d_caseB_mergecut4.root",    outdir, type, exp, stream ); // [gMC, ee or mm ]

...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  const Int_t add[Nhist][Nchain] ={
    {1},        // both leptons are true                              (ee)
    {1,1},      // one  leptons is true and the other lepton is false (ee)
    {1,1,1},    // both leptons are false                             (ee)
    {0,0,0,1},  // total                                              (ee)
    {0,0,0,0,1},       // both leptons are true                              (mm)
    {0,0,0,0,1,1},     // one  leptons is true and the other lepton is false (mm)
    {0,0,0,0,1,1,1},   // both leptons are false                             (mm)
    {0,0,0,0,0,0,0,1}, // total                                              (mm)
    {1,0,0,0,1},         // both leptons are true                              (ee+mm) 
    {1,1,0,0,1,1},       // one  leptons is true and the other lepton is false (ee+mm)
    {1,1,1,0,1,1,1},     // both leptons are false                             (ee+mm)
    {0,0,0,1,0,0,0,1},   // total                                              (ee+mm)
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    ...
    if     ( i%Ncategory== 0 ) sTmp << "( !(lpgt==3 && lmgt==3 ) && lpself == 1 && lmself==1 ) ";                                // both leptons are true                              (except cc events)
    else if( i%Ncategory== 1 ) sTmp << "( !(lpgt==3 && lmgt==3 ) && ((lpself==1 && lmself==0) || (lpself==0 && lmself==1)) ) ";  // one  leptons is true and the other lepton is false (except cc events)
    else if( i%Ncategory== 2 ) sTmp << "( !(lpgt==3 && lmgt==3 ) && lpself == 0 && lmself==0 ) ";                                // both leptons are false                             (except cc events)
    else if( i%Ncategory== 3 ) sTmp << " 1 ";                                                                                    // total
    sTmp << " && " << LRNB_cut;
  ...
  for( Int_t i=0; i<Nchain/2; i++ ){
    if     ( i== 0 ) std::cout << std::setw(40) << std::right << "lepton(t,t) except charmonium events ";
    else if( i== 1 ) std::cout << std::setw(40) << std::right << "lepton(t,f) except charmonium events ";
    else if( i== 2 ) std::cout << std::setw(40) << std::right << "lepton(f,f) except charmonium events ";
    else if( i== 3 ) std::cout << std::setw(40) << std::right << "Total";
    std::cout << " || "
  ...
  for( Int_t i=0; i<3; i++ ){
    ...
    legend1[i]->AddEntry( hist[Nplot*i+2],"lep(f,f)",   "P" );
    legend1[i]->AddEntry( hist[Nplot*i+1],"lep(t,f)",   "P" );
    legend1[i]->AddEntry( hist[Nplot*i+0],"lep(t,t)",   "P" );
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_bb_lepself1_s0%s.eps",  axis, stream) );
    c1->Print( Form("pic/%s_bb_lepself1_s0%s.root", axis, stream) );
  }

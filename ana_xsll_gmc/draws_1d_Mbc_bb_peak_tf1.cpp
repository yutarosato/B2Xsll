...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<3; i++ ){
    ...
    legend1[i]->AddEntry( hist[Nplot*i+4],"charmonium",              "P" );
    legend1[i]->AddEntry( hist[Nplot*i+3],"double false",            "P" );
    legend1[i]->AddEntry( hist[Nplot*i+2],"single false",            "P" );
    legend1[i]->AddEntry( hist[Nplot*i+1],"semi-leptonic B",         "P" );
    legend1[i]->AddEntry( hist[Nplot*i+0],"continuum",               "P" );
    c1->cd(i+1);
    waku      ->Draw();
    ar1       ->Draw();
    legend1[i]->Draw();
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_peak_tf1_s0%s.eps",  axis, stream) );
    c1->Print( Form("pic/%s_peak_tf1_s0%s.root", axis, stream) );
  }

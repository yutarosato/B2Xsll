...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  TCut cut_kfbcl = "1"; // changed at through_cut7.cpp
  ...
  std::cout << Form( "fl_rd(%d), fl_emu(%d) : ", fl_rd, fl_emu)
	    ...
	    << std::endl;
  // --------------------------------------------------------------------
  if     ( fl_rd && fl_emu ) sTmp << Form( "%s/RD_e0%s_caseB_emu_mergecut7.root",         outdir,       exp         ); // [RD,    emu    ]
  else if( fl_rd           ) sTmp << Form( "%s/RD_e0%s_caseB_mergecut7.root",             outdir,       exp         ); // [RD,  ee or mm ]
  else if(          fl_emu ) sTmp << Form( "%s/gMC_%s_e0%s_s0%d_caseB_emu_mergecut7.root",outdir, type, exp, stream ); // [gMC,   emu    ]
  else                       sTmp << Form( "%s/gMC_%s_e0%s_s0%d_caseB_mergecut7.root",    outdir, type, exp, stream ); // [gMC, ee or mm ]

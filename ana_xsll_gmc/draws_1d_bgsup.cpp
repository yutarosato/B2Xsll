...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(argc==3 || argc==4) ) std::cerr << "wrong input" << std::endl
					<< " Usage   : ./draws (char*)stream (int)fl_axis [(int)fl_appRun]" << std::endl
					<< "[fl_axis] 0(evis) 1(mmiss) 2(deroe) 3(kfbchi) 4(dzll) 5(cos-theta_B) 6(Fmiss)"
					<< std::endl, abort();
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  const Int_t Nchain        = 8; // uds(e,mu), charm(e,mu), mixed(e,mu), charged(e,mu)
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    ...
    sTmp << indir << "/gMC_" << bkgtype[i/2] << "_*_s0[" << stream << "]";
  ...
  const Int_t fl_mode_ll[Nchain] = {1,0,1,0,1,0,1,0};
  ...
  const Int_t   sel_fun[]  = {0,0,0,1092,3,0,10,10};
  ...
  std::cout << std::endl
	    << " ************************ make hist *************************************" << std::endl;
  hist[0] = new TH1D( Form("pdf_hist_%s_qq_bkg",  fname[fl_axis]), Form("pdf_hist_%s_qq_s0%s_bkg",  fname[fl_axis],stream),  xbin[fl_axis],offset[fl_axis]+xmin[fl_axis],offset[fl_axis]+xmax[fl_axis] );
  hist[1] = new TH1D( Form("pdf_hist_%s_bb_bkg",  fname[fl_axis]), Form("pdf_hist_%s_bb_s0%s_bkg",  fname[fl_axis],stream),  xbin[fl_axis],offset[fl_axis]+xmin[fl_axis],offset[fl_axis]+xmax[fl_axis] );
  hist[2] = new TH1D( Form("pdf_hist_%s_tot_bkg", fname[fl_axis]), Form("pdf_hist_%s_tot_s0%s_bkg", fname[fl_axis],stream),  xbin[fl_axis],offset[fl_axis]+xmin[fl_axis],offset[fl_axis]+xmax[fl_axis] );
  for( Int_t i=0; i<Nhist; i++ ){
    //hist[i] = new TH1D( Form("pdf_hist_%s_bkg",fname[fl_axis]),Form("pdf_hist_%s_s0%s_bkg",fname[fl_axis],stream), xbin[fl_axis],offset[fl_axis]+xmin[fl_axis],offset[fl_axis]+xmax[fl_axis] );
  ...
  // +++++++ make fit-func ++++++++++++++++++++++++++++++++++
  if(flag_fit){
    func[0] = new TF1( Form("pdf_func_%s_qq_bkg",  fname[fl_axis]), make_func(sel_fun[fl_axis]),offset[fl_axis]+xmin_fit[fl_axis],offset[fl_axis]+xmax_fit[fl_axis],n_fitfunc_par(sel_fun[fl_axis]) );
    func[1] = new TF1( Form("pdf_func_%s_bb_bkg",  fname[fl_axis]), make_func(sel_fun[fl_axis]),offset[fl_axis]+xmin_fit[fl_axis],offset[fl_axis]+xmax_fit[fl_axis],n_fitfunc_par(sel_fun[fl_axis]) );
    func[2] = new TF1( Form("pdf_func_%s_tot_bkg", fname[fl_axis]), make_func(sel_fun[fl_axis]),offset[fl_axis]+xmin_fit[fl_axis],offset[fl_axis]+xmax_fit[fl_axis],n_fitfunc_par(sel_fun[fl_axis]) );
    for( Int_t i=0; i<Nhist; i++ ){
      //func[i] = new TF1( Form("pdf_func_%s_bkg",fname[fl_axis]), make_func(sel_fun[fl_axis]),offset[fl_axis]+xmin_fit[fl_axis],offset[fl_axis]+xmax_fit[fl_axis],n_fitfunc_par(sel_fun[fl_axis]) );
      func[i]->SetLineColor(i+1);
      if( sel_fun[fl_axis]==1092 ){
	func[i]->SetParNames( "area1","alpha1","n1","area2","alpha2", "n2" );
	func[i]->SetParameters(  0.019,  0.36,  1.9, 0.11,    2.0,    3.1  );
      }else{
	func_set_parameters(sel_fun[fl_axis], func[i], hist[i], xbin[fl_axis], offset[fl_axis]+xmin[fl_axis], offset[fl_axis]+xmax[fl_axis]);
      }
    }
  }
  // +++++++ draw ++++++++++++++++++++++++++++++++++
  ...
  c1->Update();
  if( flag_save ){
    c1->Print(     Form("pic/pdf_%s_s0%s_bkg.eps",  fname[fl_axis], stream)             );
    TFile outfile( Form("pic/pdf_%s_s0%s_bkg.root", fname[fl_axis], stream), "RECREATE" );
    c1->Write();

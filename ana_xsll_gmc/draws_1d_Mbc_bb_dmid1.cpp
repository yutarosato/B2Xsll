...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    ...
    sTmp << " !(lpgt==3 && lmgt==3 ) && lpself == 0 && lmself==0 && "; // select double-misID events except CC events
    if     ( i%Ncategory== 0 ) sTmp << "  (rest_sw==0 && gm_bg1<10) && lporg==lmorg";
    else if( i%Ncategory== 1 ) sTmp << " !(rest_sw==0 && gm_bg1<10) && lporg==lmorg";
    else if( i%Ncategory== 2 ) sTmp << "                               lporg!=lmorg";
  ...
  for( Int_t i=0; i<3; i++ ){
    ...
    legend1[i]->AddEntry( hist[Nplot*i+2],"   cross",        "P" );
    legend1[i]->AddEntry( hist[Nplot*i+1],"   rest&bkg-trk", "P" );
    legend1[i]->AddEntry( hist[Nplot*i+0],"no-rest&bkg-trk", "P" );
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_bb_dmid1_s0%s.eps",  axis, stream) );
    c1->Print( Form("pic/%s_bb_dmid1_s0%s.root", axis, stream) );
  }

...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    ...
    sTmp<< "genbfl==0 || (genbfl!=0 && !(lpgt==3&&lmgt==3) && lpself==1 && lmself==1)"; // qq + semi-leptonic B decay // tmppppp
    /*
  ...
  if( flag_fit ){
	...
	func[i]->SetParNames  ("norm","Ebeam","shape");
	func[i]->SetParameters( 100*hist[i]->GetBinContent(1), endpoint, -300);
	if( flag_fix_end ) func[i]->FixParameter( 1, endpoint );
	else               func[i]->SetParLimits( 1, 5.285, 5.290 );
      }else if( sel_fun==53 ){
	func[i]->SetParNames  ("norm","Ebeam","shape");
	func[i]->SetParameters( 100*hist[i]->GetBinContent(1), endpoint, -10000);
	if( flag_fix_end ) func[i]->FixParameter( 1, endpoint );
	else               func[i]->SetParLimits( 1, 5.285, 5.290 );
      }else if( sel_fun==80 ){
	func[i]->SetParNames  ("norm","Ebeam","k");
	func[i]->SetParameters( 100*hist[i]->GetBinContent(1), endpoint, -10);
	if( flag_fix_end ) func[i]->FixParameter( 1, endpoint );
	else               func[i]->SetParLimits( 1, 5.285, 5.290 );
      }else if( sel_fun==81 ){
	func[i]->SetParNames  ("norm","Ebeam","k","new");
	func[i]->SetParameters( 100*hist[i]->GetBinContent(1), endpoint, -10, 0.5);
	if( flag_fix_end ) func[i]->FixParameter( 1, endpoint );
	else               func[i]->SetParLimits( 1, 5.285, 5.290 );
      }else if( sel_fun==15 ){
	...
	Double_t area  = 0.1;
	func[i]->SetParNames  ( "area","mean","sigma","norm",                          "Ebeam","shape" );
	func[i]->SetParameters( area,   mu,    sigma,  1.5*hist[i]->GetBinContent(1),  endpoint,   -300 );
	func[i]->FixParameter( 1, 5.2794  );
	func[i]->FixParameter( 2, 0.00273 );
	if( flag_fix_end ) func[i]->FixParameter( 4, endpoint );
	else               func[i]->SetParLimits( 4, 5.285, 5.290 );
	...
	Double_t area  = 0.1;
	func[i]->SetParNames  ( "area","mean","sigma","norm",                          "Ebeam","shape" );
	func[i]->SetParameters( area,   mu,    sigma,  1.5*hist[i]->GetBinContent(1),  endpoint,   -10000 );
	func[i]->FixParameter( 1, 5.2794  );
	func[i]->FixParameter( 2, 0.00273 );
	if( flag_fix_end ) func[i]->FixParameter( 4, endpoint );
	else               func[i]->SetParLimits( 4, 5.285, 5.290 );
	...
	Double_t sigma = 0.003;
	Double_t area  = 0.1;
	func[i]->SetParNames  ( "area","mean","sigma","norm",                          "Ebeam",     "k" );
	func[i]->SetParameters( area,   mu,    sigma,  1.5*hist[i]->GetBinContent(1),  endpoint,   -0.3 );
	func[i]->FixParameter( 1, 5.2794  );
	func[i]->FixParameter( 2, 0.00273 );
	if( flag_fix_end ) func[i]->FixParameter( 4, endpoint );
	else               func[i]->SetParLimits( 4, 5.285, 5.290 );
	...
	Double_t sigma = 0.003;
	Double_t area  = 0.1;
	func[i]->SetParNames  ( "area","mean","sigma","norm",                          "Ebeam",     "k",   "new" );
	func[i]->SetParameters( area,   mu,    sigma,  1.5*hist[i]->GetBinContent(1),  endpoint,   -0.3,     0.5 );
	func[i]->FixParameter( 1, 5.2794 );
	func[i]->FixParameter( 2, 0.00273 );
	if( flag_fix_end ) func[i]->FixParameter( 4, endpoint );
	else               func[i]->SetParLimits( 4, 5.285, 5.290 );
  ...
  if( !flag_roof ){
    c1->Update();
    c1->Print( Form("pic/%s_nopeak_fit_func%d_s0%s_c1.eps", axis, sel_fun, stream) );
  ...
  for( Int_t j=0; j<Nchain; j++ ){
	...
	for( Int_t i=0; i<Nhist; i++ ){
	  obs[i]->setVal( x_obs );
	  if( add[i][j] ){
	    if( flag_scale ) data[i]->add( RooArgSet(*obs[i]), 1/scale_event_bkg );
  ...
  for( Int_t i=0; i<Nhist; i++ ){
    ...
    if( sel_fun== 50 || sel_fun== 15 ){
      arg_shape[0] = new RooRealVar( "shape", "argus shape parameter", -15.00,  -25.0,  -5.0  );
      arg_shape[1] = new RooRealVar( "shape", "argus shape parameter", -15.00,  -25.0,  -5.0  );
      arg_shape[2] = new RooRealVar( "shape", "argus shape parameter", -12.00,  -18.0,  -8.0  );
    }else if( sel_fun== 51 || sel_fun==151 ){
      arg_shape[0] = new RooRealVar( "shape", "argus shape parameter", -19.00, -25.0,  -8.0  );
      arg_shape[1] = new RooRealVar( "shape", "argus shape parameter", -8.00,  -15.0,  -3.0  );
      arg_shape[2] = new RooRealVar( "shape", "argus shape parameter", -7.00,  -15.0,  -3.0  );
      arg_new  [0] = new RooRealVar( "new",   "argus sqrt parameter",  0.550, 0.40, 0.65 );
      arg_new  [1] = new RooRealVar( "new",   "argus sqrt parameter",  0.510, 0.35, 0.60 );
      arg_new  [2] = new RooRealVar( "new",   "argus sqrt parameter",  0.470, 0.35, 0.60 );
    }else if( sel_fun== 52 || sel_fun== 152 ){
      arg_shape[0] = new RooRealVar( "shape", "argus shape parameter", -300.00,  -1000.0,  -100.0  );
      arg_shape[1] = new RooRealVar( "shape", "argus shape parameter", -300.00,  -1000.0,  -100.0  );
      arg_shape[2] = new RooRealVar( "shape", "argus shape parameter", -300.00,  -1000.0,  -100.0  );
    }else if( sel_fun== 53 || sel_fun== 153 ){
      arg_shape[0] = new RooRealVar( "shape", "argus shape parameter", -10000.00,  -30000.0,  -5000.0  );
      arg_shape[1] = new RooRealVar( "shape", "argus shape parameter", -10000.00,  -30000.0,  -5000.0  );
      arg_shape[2] = new RooRealVar( "shape", "argus shape parameter", -10000.00,  -30000.0,  -5000.0  );
    ...
    else if( sel_fun==52 || sel_fun==152 ) modargus[i] = bindPdf( Form("modargus%d",i), func_roo_modargus2,  *obs[i], *arg_end[i], *arg_shape[i]);
    else if( sel_fun==53 || sel_fun==153 ) modargus[i] = bindPdf( Form("modargus%d",i), func_roo_modargus3,  *obs[i], *arg_end[i], *arg_shape[i]);
  ...
  for( Int_t i=0; i<Nhist; i++ ){
    ...
    if     ( sel_fun==50 || sel_fun== 51 || sel_fun== 52 || sel_fun== 53 || sel_fun== 81 || sel_fun== 82 ) fit_result[i] = modargus[i]->fitTo( *data[i]             );
    else if( sel_fun==15 || sel_fun==151 || sel_fun==152 || sel_fun==153 || sel_fun==181 || sel_fun==182 ) fit_result[i] = pdf[i]     ->fitTo( *data[i], Extended() );
    //else if( sel_fun==15 || sel_fun==151 || sel_fun==181 || sel_fun==182 ) fit_result[i] = pdf[i]     ->fitTo( *data[i], Extended(), Range(offset+xmin, 5.30) ); // tmpppppp
  } 

  /*
  TCanvas* c3 = Canvas( "c3","c3", 4, 3 );
  c3->Draw();
  RooAbsReal** nll1          = new RooAbsReal*[3];
  RooPlot**    frame_profile = new RooPlot   *[3*4];
  for( Int_t i=0; i<3; i++ ){
    nll1[i] = pdf[i]->createNLL( *data[i] );
    for( Int_t j=0; j<4; j++ ){
      if( j==2 && !(sel_fun==51 || sel_fun==151) ) continue;
      else if( j==0 ) frame_profile[4*i+j] = arg_end  [i]->frame( Bins(200),Range(5.2885, 5.2892), Title("profile") );
      else if( j==1 ) frame_profile[4*i+j] = arg_shape[i]->frame( Bins( 50),Range(   -30, 0     ), Title("profile") );
      else if( j==2 ) frame_profile[4*i+j] = arg_new  [i]->frame( Bins( 50),Range(   0.3, 0.7   ), Title("profile") );
      else if( j==3 ) frame_profile[4*i+j] = nsig     [i]->frame( Bins( 50),Range(  -100, 200   ), Title("profile") );
      nll1[i]->plotOn( frame_profile[4*i+j],ShiftToZero() );
      frame_profile[4*i+j]->SetMinimum(0);
      frame_profile[4*i+j]->SetMaximum(10);
      c3->cd(4*i+j+1);
      frame_profile[4*i+j]->Draw();
    }
  }
  if( flag_save ) c3->Print( Form("pic/%s_nopeak_fit_func%d_s0%s_c3.eps",  axis, sel_fun, stream) );
  */
  ...
  for( Int_t i=0; i<Nhist; i++ ){
    ...
    if( sel_fun==50 || sel_fun==51 || sel_fun==52 || sel_fun==53 || sel_fun==81 || sel_fun==82 ){
    ...
    }else if( sel_fun==15 || sel_fun==151 || sel_fun==152 || sel_fun==153 || sel_fun==181 || sel_fun==182 ){
  ...
  if( flag_save ){
    c1->Print( Form("pic/%s_nopeak_fit_func%d_s0%s_c1.eps",  axis, sel_fun, stream) );
    c2->Print( Form("pic/%s_nopeak_fit_func%d_s0%s_c2.eps",  axis, sel_fun, stream) );
    TFile outfile( Form("pic/%s_nopeak_fit_func%d_s0%s.root",  axis, sel_fun, stream), "RECREATE" );

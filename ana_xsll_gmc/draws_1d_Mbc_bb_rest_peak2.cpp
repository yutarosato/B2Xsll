...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<Nchain; i++ ){
    ...
    /*
    if     ( i%Ncategory== 0 ) sTmp << "  (lpgt==3 && lmgt==3 ) && (lpself==1 && lmself==1) && !(korg==lporg && lporg==lmorg && abs(pi1org)==lporg && abs(pi2org)==lporg && abs(pi3org)==lporg && abs(pi4org)==lporg && rest_sw==0 && gm_bg1<3 && ( pi0org==1 || (pi0org<0.5&&rm_xs>999) || (pi0org<0.5&&rm_xs<999)))"; // except charmonium events
    else if( i%Ncategory== 1 ) sTmp << " !(lpgt==3 && lmgt==3 ) && (lpself==0 && lmself==0) && !(korg==lporg && lporg==lmorg && abs(pi1org)==lporg && abs(pi2org)==lporg && abs(pi3org)==lporg && abs(pi4org)==lporg && rest_sw==0 && gm_bg1<3) ";  // except double miss-id
    else if( i%Ncategory== 2 ) sTmp << " !(lpgt==3 && lmgt==3 ) && ( (lpself==1&&lmself==0) || (lpself==0&&lmself==1) ) && !(( lpgt==3 || lmgt==3 ) && rest_sw==0 && gm_bg1<3              && ( korg==lporg && abs(pi1org)==lporg && abs(pi2org)==lporg && abs(pi3org)==lporg && abs(pi4org)==lporg ) && lporg==lmorg) "; // except swapped miss-id
    else if( i%Ncategory== 3 ) sTmp << " !(lpgt==3 && lmgt==3 ) && ( (lpself==1&&lmself==0) || (lpself==0&&lmself==1) ) && !(  lpgt!=3 && lmgt!=3   && rest_sw==0 && gm_bg1<3 && rm_xs>999 && ( korg==lporg && abs(pi1org)==lporg && abs(pi2org)==lporg && abs(pi3org)==lporg && abs(pi4org)==lporg ) && lporg==lmorg) "; // except single miss-id with neutrino
    else if( i%Ncategory== 4 ) sTmp << " 1 "; // total -> continuum
    */

    ///*
    if( i%Ncategory== 4 ) sTmp << " 1 "; // total
    else{
      sTmp << " !(lpgt==3 && lmgt==3 ) && ( (lpself==1&&lmself==0) || (lpself==0&&lmself==1) ) "
	   << " && !(( lpgt==3 || lmgt==3 ) && rest_sw==0 && gm_bg1<3              && ( korg==lporg && abs(pi1org)==lporg && abs(pi2org)==lporg && abs(pi3org)==lporg && abs(pi4org)==lporg ) && lporg==lmorg) "; // except swapped miss-id
      if     ( i%Ncategory== 0 ) sTmp << " && 1==0"; //"rm_xs>999";
      else if( i%Ncategory== 1 ) sTmp << " && 1==0"; //"rm_xs<999"; 
      else if( i%Ncategory== 2 ) sTmp << " && rm_xs<999 && lporg==lmorg";
      else if( i%Ncategory== 3 ) sTmp << " && rm_xs<999 && lporg!=lmorg";
    }
    //*/
    sTmp << " && " << LRNB_cut;
  ...
  for( Int_t j=0; j<Nchain; j++ ){
    ...
    if( flag_scan ){
      ...
      chain[j]->GetTree()->Scan( "exprun:event:Mbc:xs_m:cc_m:cc_morg:rm_xs:pi0self:pi0org:lpgt:lmgt:lporg:lmorg:lpself:lmself:lpselfid:lmselfid:lpmoid:lmmoid:gb1_semi:gb2_semi:gb1nd:gb2nd:gb1d1_se:gb1d2_se:gb2d1_se:gb2d2_se:gm_bg1:gm_bg2:gm_b1:gm_b2:gm_l1:gm_l2:gm_nu1:gm_nu2:rest:rest_sw:rest2:rest2_sw:dntrk:ntrk:kmoid:pi1moid:pi2moid:pi3moid:pi4moid:korg:pi1org:pi2org:pi3org:pi4org:kselfid:pi1selfi:pi2selfi:pi3selfi:pi4selfi:lp_eid:lm_eid:lp_muid:lm_muid:lp_kid:lm_kid:pi1_eid:pi1_muid:pi2_eid:pi2_muid:epp:emp:mpp:mmp:pi1p:pi2p:pi3p:pi4p",
  ...
  for( Int_t i=0; i<3; i++ ){
    ...
    legend1[i]->AddEntry( hist[Nplot*i+3],"except single",  "P" );
    legend1[i]->AddEntry( hist[Nplot*i+2],"except swapped", "P" );
    legend1[i]->AddEntry( hist[Nplot*i+1],"except double",  "P" );
    legend1[i]->AddEntry( hist[Nplot*i+0],"except cc",      "P" );
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_rest_peak2_s0%s.eps",  axis, stream) );
    c1->Print( Form("pic/%s_rest_peak2_s0%s.root", axis, stream) );
  }

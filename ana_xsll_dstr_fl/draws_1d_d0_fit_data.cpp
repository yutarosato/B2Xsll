...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  TFile infile0 ( Form("pic_d0_plot_data_xdst035/d0_plot_data_pid0_par%d_mom%d_cos%d_adef%d.root", fl_par, fl_mom, fl_cos, fl_adef) ); // change the xdst cut value !
  TFile infile1 ( Form("pic_d0_plot_data_xdst035/d0_plot_data_pid1_par%d_mom%d_cos%d_adef%d.root", fl_par, fl_mom, fl_cos, fl_adef) ); // change the xdst cut value !
  TFile infile2 ( Form("pic_d0_plot_data_xdst035/d0_plot_data_pid2_par%d_mom%d_cos%d_adef%d.root", fl_par, fl_mom, fl_cos, fl_adef) ); // change the xdst cut value !
  ...
  if( hist[Nhist-1]->Integral() < 10000 ) std::cout << "[WARNING] too small sample : " << hist[Nhist-1]->Integral() << std::endl;

  for(Int_t i=0; i<Nhist; i++ ) Deco( hist[i], 3, col_fil[i%Ncategory], 1 );

  // +++++++ make fit-func ++++++++++++++++++++++++++++++++++
  Double_t slope = (hist[Nhist-1]->Integral(xbin-5,xbin) - hist[Nhist-1]->Integral(1,5) )/ (xmax-xmin);
  for( Int_t j=0; j<Nhist/Ncategory; j++ ){
    ...
    if( sel_fun==111 ){
      Double_t area = 0;
      if     ( j==0 ) area = 0.50*hist[j*Ncategory]->Integral()*hist[j*Ncategory]->GetBinWidth(0);
      else if( j==1 ) area = 0.08*hist[j*Ncategory]->Integral()*hist[j*Ncategory]->GetBinWidth(0);
      else if( j==2 ) area = 0.85*hist[j*Ncategory]->Integral()*hist[j*Ncategory]->GetBinWidth(0);

      Double_t a = (hist[j*Ncategory]->Integral(xbin-5,xbin) - hist[j*Ncategory]->Integral(1,5) )/ (xmax-xmin);
      Double_t b = hist[j*Ncategory]->Integral(1,5)/5 - a*xmin;
  ...
  for( Int_t j=Nhist/Ncategory-1; j>=0; j-- ){
    c1->cd(j+1);
    hist[j]->SetMarkerStyle(1);
    Int_t fit_result = hist[j]->Fit(func[j],"R","PE0");
	...
	if     ( cnt_v<0  ) func[2]->SetParameter(i, init_var[i]);
      ...
      if( cnt_v<0 ) fit_result = hist[j]->Fit(func[j],"R", "PE0");
      else          fit_result = hist[j]->Fit(func[j],"RQ","PE0");
  ...
  // ++++++ save fake-rate ++++++++++++++++++++
  if( !fl_err ){
    std::ofstream fout( Form("log/log_d0_fit_data_par%d_mom%d_cos%d_adef%d.dat",  fl_par, fl_mom, fl_cos, fl_adef) );
    for( Int_t j=0; j<3; j++ ){
      // Error check
      if( fake_rate[j] < 0 ){
  ...
  for( Int_t j=0; j<Nhist/Ncategory; j++ ){
    ...
    c1->cd(j+1); func_bkg[j]->Draw("same");
  }
  //*/
  // ++++++++ save +++++++++++++++++++++++++++++++++++++++++++++  
  c1->Update();
  if( flag_save ){
    c1->Print(     Form("pic/d0_fit_data_par%d_mom%d_cos%d_adef%d.eps",  fl_par, fl_mom, fl_cos, fl_adef) );
    TFile outfile( Form("pic/d0_fit_data_par%d_mom%d_cos%d_adef%d.root", fl_par, fl_mom, fl_cos, fl_adef), "RECREATE" );

...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<Nroohist; i++ ){
    ...
    for( Int_t j=0; j<Nbin_afb/2; j++ ){
      ...
      nsig_f     [i][2*j+0] = new RooFormulaVar( Form("nsig_f_%d_%d",     i,2*j+0), "@0*@1", RooArgSet(*r_f[i][2*j+0],  *nsig        [i][2*j+1]) );
      nsig_f     [i][2*j+1] = new RooFormulaVar( Form("nsig_f_%d_%d",     i,2*j+1), "@0*@1", RooArgSet(*r_f[i][2*j+1],  *nsig        [i][2*j+0]) );
      ...
      nsig_f_sim [i][2*j+0] = new RooFormulaVar( Form("nsig_f_sim_%d_%d", i,2*j+0), "@0*@1", RooArgSet(*r_f [i][2*j+0], *nsig_sim    [i][2*j+1]) );
      nsig_f_sim [i][2*j+1] = new RooFormulaVar( Form("nsig_f_sim_%d_%d", i,2*j+1), "@0*@1", RooArgSet(*r_f [i][2*j+1], *nsig_sim    [i][2*j+0]) );
    }
  }
  ...
  */
  ...
  if( flag_save ){
    TFile outfile( Form("pic/%s_fit_bin_dt_func%d_%dq2_syst_scf_f.root",  axis, sel_fun, fl_q2bin), "RECREATE" );
    for( Int_t i=0; i<Nroohist; i++ ){
    ...
    c2->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_c2_syst_scf_f.eps",  axis, sel_fun, fl_q2bin) );
    c2->Write();

    c3->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_c3_syst_scf_f.eps",  axis, sel_fun, fl_q2bin) );
    c3->Write();

...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  TH1D**** hist_scf_dummy = new TH1D***[Nroohist]; // [Nbin_afb][4]

  for( Int_t i=0; i<Nroohist; i++ ){
    hist_scf      [i] = new TH1D**[Nbin_afb];
    hist_scf_dummy[i] = new TH1D**[Nbin_afb];
    for( Int_t j=0; j<Nbin_afb; j++ ){
      hist_scf      [i][j] = new TH1D*[4]; // true,(t,t), (t,f), (f,?)
      hist_scf_dummy[i][j] = new TH1D*[4]; // true,(t,t), (t,f), (f,?)
      for( Int_t k=0; k<4; k++ ){
	Int_t m = k+4*(j+i*Nbin_afb);
	hist_scf_dummy[i][j][k] = (TH1D*)file_scf.Get( Form("hist%d",m) );
	hist_scf_dummy[i][j][k]->SetName ( Form("hist_dummy%d",m) );
	hist_scf_dummy[i][j][k]->SetTitle( Form("hist_dummy%d",m) );
	hist_scf      [i][j][k] = new TH1D( Form("hist%d",m), Form("hist%d",m), 32, 5.22, 5.30 );
	for( Int_t m=0; m<10000; m++ ) hist_scf[i][j][k]->Fill( hist_scf_dummy[i][j][k]->GetRandom() );
      }
    }
  }
  ...
  */
  ...
  if( flag_save ){
    TFile outfile( Form("pic/%s_fit_bin_dt_func%d_%dq2_scf_cnt%d.root",  axis, sel_fun, fl_q2bin, fl_cnt), "RECREATE" );
    for( Int_t i=0; i<Nroohist; i++ ){
    ...
    c2->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_c2_scf_cnt%d.eps",  axis, sel_fun, fl_q2bin, fl_cnt) );
    c2->Write();

    c3->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_c3_scf_cnt%d.eps",  axis, sel_fun, fl_q2bin, fl_cnt) );
    c3->Write();

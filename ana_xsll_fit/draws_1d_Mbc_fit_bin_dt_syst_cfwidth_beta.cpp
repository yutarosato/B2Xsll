...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(argc==4 || argc==5) ) std::cerr << "wrong input" << std::endl
					<< " Usage : ./draws_1d_Mbc_fit (int)sel_fun (int)fl_q2bin (int)fl_cnt [(int)fl_appRun]"     << std::endl
  ...
  {
    Double_t tmp_rnd = gRandom->Gaus(0, 1);
    cf_offset[ 0][ 0] = new RooRealVar( "cfo_0_0", "cfo_0_0",    0.0  *tmp_rnd );
    cf_offset[ 0][ 1] = new RooRealVar( "cfo_0_1", "cfo_0_1",    0.008997   *tmp_rnd );
    cf_offset[ 0][ 2] = new RooRealVar( "cfo_0_2", "cfo_0_2",    0.00348185 *tmp_rnd );
    cf_offset[ 0][ 3] = new RooRealVar( "cfo_0_3", "cfo_0_3",    0.0  *tmp_rnd );
    cf_offset[ 1][ 0] = new RooRealVar( "cfo_1_0", "cfo_1_0",    0.0  *tmp_rnd );
    cf_offset[ 1][ 1] = new RooRealVar( "cfo_1_1", "cfo_1_1",    0.0  *tmp_rnd );
    cf_offset[ 1][ 2] = new RooRealVar( "cfo_1_2", "cfo_1_2",    0.0  *tmp_rnd );
    cf_offset[ 1][ 3] = new RooRealVar( "cfo_1_3", "cfo_1_3",    0.0  *tmp_rnd );
  }
  for( Int_t i=0; i<Nroohist; i++ ){
    AFB_meas[i] = new RooFormulaVar*[Nbin_afb];
    for( Int_t j=0; j<Nbin_afb/2; j++ ){
      if(      i==0 && j==1 ) AFB_meas[i][j] = new RooFormulaVar( Form("A_{FB}^{meas.}_%d_%d",i,j),  "1.01868*(@1/@0-@2*sqrt(1+1.01868*1.01868))", RooArgSet(*cf_slope[i][j], *AFB_true[j], *cf_offset[i][j]) ); // @ 20130924
      else if( i==0 && j==2 ) AFB_meas[i][j] = new RooFormulaVar( Form("A_{FB}^{meas.}_%d_%d",i,j),  "1.00340*(@1/@0-@2*sqrt(1+1.00340*1.00340))", RooArgSet(*cf_slope[i][j], *AFB_true[j], *cf_offset[i][j]) ); // @ 20130924
      else                    AFB_meas[i][j] = new RooFormulaVar( Form("A_{FB}^{meas.}_%d_%d",i,j),  "1.0/@0*@1",     RooArgSet(*cf_slope[i][j], *AFB_true[j]) ); // @ 20130924
    }
  }
  ...
  */
  ...
  if( flag_save ){
    TFile outfile( Form("pic/%s_fit_bin_dt_func%d_%dq2_cfwidth_cnt%d.root",  axis, sel_fun, fl_q2bin, fl_cnt), "RECREATE" );
    for( Int_t i=0; i<Nroohist; i++ ){
    ...
    c2->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_c2_cfwidth_cnt%d.eps",  axis, sel_fun, fl_q2bin, fl_cnt) );
    c2->Write();

    c3->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_c3_cfwidth_cnt%d.eps",  axis, sel_fun, fl_q2bin, fl_cnt) );
    c3->Write();

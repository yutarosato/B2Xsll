...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  strcpy( file_afb,  Form("tmp%d_afb.dat",  tmp_i) );
  ...
  const Double_t incl_ratio_max = 2.0;
  //++++++++++++++++++++++++++++++
  ...
  for( Int_t i=0; i<nbin_q2; i++ ){
    l_afb_gen[i] = new TLine(incl_ratio_min, AFB_gen_6qbin[tmp_i][i], incl_ratio_max, AFB_gen_6qbin[tmp_i][i]);
    l_afb_gen[i]->SetLineColor(3);
    l_afb_gen[i]->SetLineStyle(7);
    l_afb_eff[i] = new TLine(incl_ratio_min, AFB_eff_6qbin[tmp_i][i], incl_ratio_max, AFB_eff_6qbin[tmp_i][i]);
  ...
  for( Int_t i=0; i<nbin_q2; i++ ){
    ...
    g_nq2[i]->GetXaxis()->SetTitle("incl ratio" );
  ...
  g_ntot->GetXaxis()->SetTitle("incl ratio");
  ...
  legend1->AddEntry( f_afb[0],     "Rec(Fit->Eff)", "L" );
  legend1->Draw();

  // SAVE
  if( fl_save ) c1->Print( Form("pic/linearity_check_bin_%d.eps",tmp_i) );

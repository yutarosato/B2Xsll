...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<Nbin_afb; i++ ){
	...
	sTmp << " && ( "
	     ...
	     << " || "
	     << makeCut_q2( fl_mode_ll[j], 3,  1 ).c_str()
	     << " || "
	     << makeCut_q2( fl_mode_ll[j], 5,  1 ).c_str()
	     << " || "
  ...
  if( flag_save ){
    for( Int_t i=0; i<Nplot; i++ ) c1[i]->Print( Form("pic/%s_self_cf2_totFB_set%s_%d.eps",  axis, setname, i+1) );
    TFile outfile( Form("pic/%s_self_cf2_totFB_set%s.root", axis, setname), "RECREATE" );

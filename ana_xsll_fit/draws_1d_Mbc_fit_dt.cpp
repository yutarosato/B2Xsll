...
const Bool_t   flag_cc       = true; // 1(cc-MC), 0(gMC) for charmonium peaking PDF
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !flag_roof ){
    c1->Update();
    c1->Print( Form("pic/%s_fit_func%d_c1.eps", axis, sel_fun) );
  ...
  TFile file_scf_afb(    "pdf/Mbc_self_cf_tot_setA-U.root");
  TFile file_scf_all("pdf_all/Mbc_self_cf_tot_setA-U.root");
  if( file_scf_afb.IsZombie() ) std::cerr << "[ABORT] can not find input-file : " << file_scf_afb.GetName() << std::endl, abort();
  if( file_scf_all.IsZombie() ) std::cerr << "[ABORT] can not find input-file : " << file_scf_all.GetName() << std::endl, abort();
  
  TH1D** hist_scf = new TH1D*[12];
  for( Int_t i=0; i<12; i++ ){
    if( flag_k4pi && flag_unflavor ) hist_scf[i] = (TH1D*)file_scf_afb.Get( Form("hist%d",i) ); 
    else                             hist_scf[i] = (TH1D*)file_scf_all.Get( Form("hist%d",i) ); 
  ...
  if( flag_k4pi && flag_unflavor ){ // AFB-modes
  ...
  for( Int_t i=0; i<Nroohist; i++ ){
    Double_t tmp_nsig;
    if     ( i==0 ) tmp_nsig = 150;
    else if( i==1 ) tmp_nsig = 150;
    else if( i==2 ) tmp_nsig = 300;
    nsig[i] = new RooRealVar ( "N_{sig}", "N_{sig}", tmp_nsig  ,                     tmp_nsig-200,                       tmp_nsig+200                       );
    nbkg[i] = new RooRealVar ( "N_{bkg}", "N_{bkg}", data[i]->sumEntries()-tmp_nsig, data[i]->sumEntries()-tmp_nsig-200, data[i]->sumEntries()-tmp_nsig+200 );
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  // READ PDF (charmonium)
  TFile file_cc_cc ("pdf/Mbc_peak_cc_s00.root"  ); // CC-MC
  TFile file_cc_gmc("pdf/Mbc_peak_cc_s00-5.root"); // gMC
  if( file_cc_cc.IsZombie () ) std::cerr << "[ABORT] can not find input-file : " << file_cc_cc.GetName()  << std::endl, abort();
  if( file_cc_gmc.IsZombie() ) std::cerr << "[ABORT] can not find input-file : " << file_cc_gmc.GetName() << std::endl, abort();
  
  TH1D** hist_cc = new TH1D*[(Nbin_afb+1)*3];
  for( Int_t i=0; i<(Nbin_afb+1)*3; i++ ){
    if( flag_cc ) hist_cc[i] = (TH1D*)file_cc_cc.Get ( Form("hist%d",i) );
    else          hist_cc[i] = (TH1D*)file_cc_gmc.Get( Form("hist%d",i) );
  ...
  if( flag_k4pi && flag_unflavor ){ // AFB-modes(RD)
  ...
  if( flag_save ){
    c1->Print( Form("pic/%s_fit_func%d_c1.eps",  axis, sel_fun) );
    c2->Print( Form("pic/%s_fit_func%d_c2.eps",  axis, sel_fun) );
  }

  for( Int_t i=0; i<Nroohist; i++ ){
    std::cout << std::setw(10) << std::right << nsig[i]->getVal()                             << " "

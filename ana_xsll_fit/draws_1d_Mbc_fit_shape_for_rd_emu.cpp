...
const Char_t* indir = "~/ewp/ana/ana_xsll_comb/hbk_afb_calib/hbk_cut2/rd_emu_522/"; // emu
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  const Int_t Nchain        = 1; // [emu(rd)]
  const Int_t Ncategory     = 1;
  const Int_t Nplot         = 1;
  const Int_t Ntmp          = Ncategory*1;
  const Int_t Nhist         = Nplot    *1;
  const Int_t fl_sb[Ntmp]   = {0};      // 0(rd,emu)
  const Int_t fl_mode_ll[Nchain] = {2}; // 1(e), 0(mu), 2(emu)
  ...
  sTmp << Form( "%s/RD_*_emu0", indir);
  //sTmp << Form( "%s/RD_*_emu1", indir); // tmpppp
  ...
  for( Int_t j=0; j<Nchain; j++ ){
    ...
    chain[j] = new MChain( infile, tname, branch_table(), 0, tail );
  ...
  for( Int_t i=0; i<Nhist; i++ ){
    ...
    Deco( hist[i], 3, col_fil[i%Nplot+3], col_fil[i%Nplot+3] );
    for( Int_t j=0; j<Ntmp; j++ ){
  ...
  waku[0] = Waku( Nhist, hist, Form("%s (e#mu)",   xlabel) );
  
  for(Int_t i=Nhist-1; i>=0; i-- ){
    c1->cd(i/Nplot+1);
    if     ( 0*Nplot <= i && i < 1*Nplot ) hist[i]->SetTitle( Form("e#mu"  ) );
  ...
  if( !flag_roof ){
    c1->Update();
    c1->Print( Form("pic/%s_fit_shape_rd_emu_%dbin_%d_func%d_c1.eps", axis, fl_nbin, fl_bin, sel_fun) );
  ...
  // --- Observable ---
  const Int_t    Nroohist        = 1; // emu
  const Int_t    rooad[Nroohist] = {0};
  ...
  for( Int_t i=0; i<Nroohist; i++ ){
    ...
    arg_shape[i] = new RooRealVar( "shape", "argus shape parameter", tmp_shape, tmp_shape-40.0, tmp_shape+30.0 );
  ...
  if( flag_save ){
    c1->Print( Form("pic/%s_fit_shape_rd_emu_%dbin_%d_func%d_c1.eps",  axis, fl_nbin, fl_bin, sel_fun) );
    c2->Print( Form("pic/%s_fit_shape_rd_emu_%dbin_%d_func%d_c2.eps",  axis, fl_nbin, fl_bin, sel_fun) );
  }

...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  */
  ...
  if( flag_save ){
    TFile outfile( Form("pic/%s_fit_bin_dt_func%d_%dq2_cc_gmc.root",  axis, sel_fun, fl_q2bin), "RECREATE" );
    for( Int_t i=0; i<Nroohist; i++ ){
    ...
    c2->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_cc_gmc_c2.eps",  axis, sel_fun, fl_q2bin) );
    c2->Write();

    c3->Print( Form("pic/%s_fit_bin_dt_func%d_%dq2_cc_gmc_c3.eps",  axis, sel_fun, fl_q2bin) );
    c3->Write();

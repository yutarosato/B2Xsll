...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t i=0; i<Nbin_afb; i++ ){
    ...
    for( Int_t j=0; j<Ntmp; j++ ){
      ...
      if     ( i== 0 ){
	sTmp << " && ( "
	     << makeCut_q2( fl_mode_ll[j], 1,  1 ).c_str()
	     << " || "
	     << makeCut_q2( fl_mode_ll[j], 2,  1 ).c_str()
	     << " )";
      }else if( i==1 ){
	sTmp << " && ( "
	     << makeCut_q2( fl_mode_ll[j], 1, -1 ).c_str()
	...
	sTmp << " && " << makeCut_q2( fl_mode_ll[j], 3,  1 ).c_str();
      }else if( i==3 ){
	sTmp << " && " << makeCut_q2( fl_mode_ll[j], 3, -1 ).c_str();
      }else if( i==4 ){
	sTmp << " && " << makeCut_q2( fl_mode_ll[j], 5,  1 ).c_str();
      }else if( i==5 ){
	sTmp << " && " << makeCut_q2( fl_mode_ll[j], 5, -1 ).c_str();
      }else if( i==6 ){
	sTmp << " && ( "
	     << makeCut_q2( fl_mode_ll[j], 7,  1 ).c_str()
	     << " || "
	     << makeCut_q2( fl_mode_ll[j], 8,  1 ).c_str()
	     << " || "
	     << makeCut_q2( fl_mode_ll[j], 9,  1 ).c_str()
	     << " ) ";
      }else if( i==7 ){
	sTmp << " && ( "
	     << makeCut_q2( fl_mode_ll[j], 7, -1 ).c_str()
  ...
  for( Int_t i=0; i<Nplot; i++ ){
    c1[i] = Canvas( Form("c1_%d",i+1), Form("c1_%d",i+1), 4, 2 );
    c1[i]->Draw();
  }
  ...
  if( flag_save ){
    for( Int_t i=0; i<Nplot; i++ ) c1[i]->Print( Form("pic/%s_self_cf2_set%s_%d.eps",  axis, setname, i+1) );
    TFile outfile( Form("pic/%s_self_cf2_set%s.root", axis, setname), "RECREATE" );

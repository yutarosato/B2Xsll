...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(argc==6 || argc==7) ) std::cerr << "wrong input" << std::endl
    					<< " Usage : ./draws_1d_Mbc (char*)stream (char*) setname (double)used_nstream (double)used_nset (int)fl_body [(int)fl_appRun]" << std::endl
					<< std::endl, abort();
  ...
  c1->Update();
  if( flag_save ){
    c1->Print( Form("pic/%s_%dbody_s0%s_set%s_c1.eps",  axis, fl_body, stream, setname) );
  }

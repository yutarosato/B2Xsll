...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  using namespace bgsup_var_calib;
  ...
  for( Int_t j=0; j<Nchain; j++ ){
    ...
    if     ( flag_calib && fl_axis==4 && fl_sb[j]!=2                     ) chain[j]->GetTree()->Project( Form("tmphist%d",j), Form("1.28*%s",       axis[fl_axis]), add_cut[j] ); // dzll3d
    else if( flag_calib && fl_axis==6 && fl_sb[j]!=2 && fl_mode_ll[j]==1 ) chain[j]->GetTree()->Project( Form("tmphist%d",j), Form("1.13*%s-0.0019",axis[fl_axis]), add_cut[j] ); // delta-E(ee)
    else if( flag_calib && fl_axis==6 && fl_sb[j]!=2 && fl_mode_ll[j]==0 ) chain[j]->GetTree()->Project( Form("tmphist%d",j), Form("1.15*%s-0.0009",axis[fl_axis]), add_cut[j] ); // delta-E(mm)
    else if( flag_calib && fl_axis==2 && fl_sb[j]!=2                     ) chain[j]->GetTree()->Project( Form("tmphist%d",j), Form("1.07*%s",       axis[fl_axis]), add_cut[j] ); // chi2(b-vtx)

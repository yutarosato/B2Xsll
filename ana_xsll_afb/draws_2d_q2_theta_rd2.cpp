...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  const Int_t    Nchain          = 4; // [2 Mbc sideband] x[gmc, rd]
  const Int_t    Nhist           = 4; // [2 Mbc sideband] x[gmc, rd]
  ...
  const Int_t    fl_rd[Nchain]   = {0,0,1,1}; // 0(gmc), 1(rd)
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  // cut change
  for( Int_t j=0; j<Nchain; j++ ){
    if     ( j%(Nchain/2)==0 ) chain[j]->GetCut()->Set( "Mbc", 1,  5.22, 0.0, 5.24 ); // Mbc sideband(low)
    else if( j%(Nchain/2)==1 ) chain[j]->GetCut()->Set( "Mbc", 1,  5.24, 0.0, 5.26 ); // Mbc sideband(high)
  }
  // ++++++++++++++++++++++++
  std::cout << std::endl
  ...
  for( Int_t i=0; i<Nhist; i++ ){
    ...
    if     ( i== 0 ) hist2[i]->SetTitle("gmc, sideband(low)"  );
    else if( i== 1 ) hist2[i]->SetTitle("gmc, sideband(high)" );
    else if( i== 2 ) hist2[i]->SetTitle("rd,  sideband(low)"  );
    else if( i== 3 ) hist2[i]->SetTitle("rd,  sideband(high)" );
  ...
  legend1->AddEntry( histx[0], "gmc, sideband(low)",  "L" );
  legend1->AddEntry( histx[1], "gmc, sideband(high)", "L" );
  legend1->AddEntry( histx[2], "rd,  sideband(low)",  "L" );
  legend1->AddEntry( histx[3], "rd,  sideband(high)", "L" );
  ...
  if( flag_save ){
    c1->Print( Form("pic/2d_q2_theta_rd2_lep%d_s0%s_c1.eps", fl_mode_ll, stream) );
    c2->Print( Form("pic/2d_q2_theta_rd2_lep%d_s0%s_c2.eps", fl_mode_ll, stream) );
  }
  ...
  legend2->AddEntry( histx[0], "gmc, sideband(low)",  "L" );
  legend2->AddEntry( histx[1], "gmc, sideband(high)", "L" );
  legend2->AddEntry( histx[2], "rd,  sideband(low)",  "L" );
  legend2->AddEntry( histx[3], "rd,  sideband(high)", "L" );

  legend2->Draw();
  c3->Update();

  if( flag_save ){
    c3->Print( Form("pic/2d_q2_theta_rd2_lep%d_s0%s_c3.eps", fl_mode_ll, stream) );
  }
